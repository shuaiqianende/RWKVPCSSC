...
# wkv_cuda = load(name="wkv", sources=["src/models/components/cuda/wkv_op.cpp", "src/models/components/cuda/wkv_cuda.cu"],
#                 verbose=True, extra_cuda_cflags=['-res-usage', '--maxrregcount 128', '--use_fast_math', '-O3', '-Xptxas -O3', f'-DTmax={T_MAX}'])
wkv_cuda = load(name="wkv", sources=["src/models/components/cuda/wkv_op.cpp", "src/models/components/cuda/wkv_cuda.cu"],
                verbose=True,
...
def serial_shift(input, shift_points=1, paragraph_count=16, gamma=4):
    B, N, C = input.shape
    assert N % paragraph_count == 0, "N must be divisible by paragraph_count"
    assert C % gamma == 0, "C must be divisible by gamma"
    para_len = N // paragraph_count
    para_C = C // gamma
    input = input.transpose(1, 2).reshape(B, C, paragraph_count, para_len)
    output = torch.zeros_like(input)
    for i in range(4):
        if i % 2 == 0:
            output[:, int(para_C * i):int(para_C * (i + 1)), :, shift_points:para_len] = input[:, int(para_C * i):int(
                para_C * (i + 1)), :, 0:para_len - shift_points]
        else:
            output[:, int(para_C * i):int(para_C * (i + 1)), :, 0:para_len - shift_points] = input[:,
                                                                                             int(para_C * i):int(
                                                                                                 para_C * (i + 1)), :,
                                                                                             shift_points:para_len]
    return output.flatten(2).transpose(1, 2)


class VRWKV_SpatialMix(nn.Module):
    def __init__(self, n_embd, n_layer, layer_id, shift_mode='serial_shift',
                 channel_gamma=1 / 4, shift_points=1, init_mode='fancy',
    ...
    def jit_func(self, x, patch_resolution):
        ...
        B, T, C = x.size()
        if self.shift_points > 0:
            xx = self.shift_func(x)
...
class VRWKV_ChannelMix(nn.Module):
    def __init__(self, n_embd, n_layer, layer_id, shift_mode='serial_shift',
                 channel_gamma=1 / 4, shift_points=1, hidden_rate=4, init_mode='fancy',
                 key_norm=False, with_cp=False):
        super().__init__()
        self.layer_id = layer_id
        self.n_layer = n_layer
...
class Block(nn.Module):
    def __init__(self, n_embd, n_layer, layer_id, shift_mode='serial_shift',
                 channel_gamma=1 / 4, shift_points=1, drop_path=0.2, hidden_rate=4,
        ...
        self.att = VRWKV_SpatialMix(n_embd, n_layer, layer_id, shift_mode,
                                    channel_gamma, shift_points, init_mode,
                                    key_norm=key_norm)

        self.ffn = VRWKV_ChannelMix(n_embd, n_layer, layer_id, shift_mode,
                                    channel_gamma, shift_points, hidden_rate,
