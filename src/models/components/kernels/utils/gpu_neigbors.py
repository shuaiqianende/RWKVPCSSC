...
from src.models.components.kernels.utils.batch_conversion import batch_to_pack, pack_to_batch, pack_to_list, list_to_pack
...
def knn(q_points: Tensor,
    ...
    if distance_limit is not None:
        ...
        if padding_mode == "nearest":
            knn_distances[knn_masks] = knn_distances[..., 0].unsqueeze(-1).expand(-1, -1, knn_distances.shape[2])[knn_masks]
            knn_indices[knn_masks] = knn_indices[..., 0].unsqueeze(-1).expand(-1, -1, knn_distances.shape[2])[knn_masks]
        else:
