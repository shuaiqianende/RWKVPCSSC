...
#define IDEAL_T_LEN (Tmax_LOCAL / TOKEN_SPLIT)


template <typename F>
__global__ void kernel_forward_local(const int B, const int T, const int C,
...
template <typename F>
__global__ void kernel_backward_local(const int B, const int T, const int C,
...
void cuda_forward_local(int B, int T, int C, float *w, float *u, float *k, float *v, float *y) {
    ...
    kernel_forward_local<<<numBlocks, threadsPerBlock>>>(B, T, C, w, u, k, v, y);
}
...
void cuda_backward_local(int B, int T, int C, float *w, float *u, float *k, float *v, float *gy, float *gw, float *gu, float *gk, float *gv) {
    ...
    assert(B * C % threadsPerBlock.x == 0);
    dim3 numBlocks(B * C / threadsPerBlock.x);
    kernel_backward_local<<<numBlocks, threadsPerBlock>>>(B, T, C, w, u, k, v, gy, gw, gu, gk, gv);
}
