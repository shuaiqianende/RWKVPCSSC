...
from src.models.components.kernels.kernel_points import load_kernels
from src.models.components.generic_blocks import gather, index_select, radius_gaussian, local_maxpool, UnaryBlock, \
...
class _KPNextBlock(nn.Module):
    ...
    def __init__(self,
        ...
        """
        super(_KPNextBlock, self).__init__()
        ...
        if in_channels != out_channels:
            self.up_mlp = nn.Sequential(nn.Linear(in_channels, out_channels, bias=False),
                                        ...
                                        activation)
        self.up_down_mlp = nn.Sequential(nn.Linear(out_channels, out_channels * 4, bias=False),
                                        NormBlock(out_channels * 4, norm_type, bn_momentum),
                                        activation,
    ...
    def forward(self, q_pts, s_pts, x, neighbor_indices):
        ...
        if self.mlp_first:
            if self.in_channels != self.out_channels:
                ...
                _x = x
            x = self.conv(q_pts, s_pts, _x, neighbor_indices)
            x = self.conv_norm(x)
            x = self.activation(x)
            x = self.up_down_mlp(x) + _x
        #  Option 2: Upscale features last
        else:
            _x = x
            x = self.conv(q_pts, s_pts, x, neighbor_indices)
            x = self.conv_norm(x)
            x = self.activation(x) + _x
            if self.in_channels != self.out_channels:
                x = self.up_mlp(x)
