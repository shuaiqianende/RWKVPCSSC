...
class SSCDataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        # sampler = DistributedSampler(self.data_train)  # 使用分布式采样器
        # sampler = DistributedSampler(self.data_train, num_replicas=torch.cuda.device_count(), rank=torch.distributed.get_rank())
        return DataLoader(
            ...
            shuffle=True,
            # shuffle=False,
            # sampler=sampler,  # 将采样器传递给 DataLoader
        )
