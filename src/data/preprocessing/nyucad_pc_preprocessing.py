...
class NYUCADPreprocessing(BasePreprocessing):
    def __init__(
        ...
        data_dir='/Path/NYUCAD-PC/',
        save_dir='/Path/NYUCAD-PC_v2/',
    ...
    def is_point_in_triangle(self, pt, v0, v1, v2):
        # 计算向量
        v0v1 = v1 - v0
        v0v2 = v2 - v0
        v0pt = pt - v0
    ...
    def process_file(self, filepath, mode):
        ...
        gt_pcd, gt_pcd_crop, gt_mesh_o3d, raw_label, *trans_param = self._gt_compile(scene_model)
        gt_mesh_colored_o3d = self._mesh_colored_o3d(gt_mesh_o3d)
        # 对顶点进行重新排列缩放
        gt_mesh_colored_o3d.vertices = o3d.utility.Vector3dVector(
            np.asarray(gt_mesh_colored_o3d.vertices)[:, [1, 2, 0]] / self.ds
        )
        ...
        for i, triangle in enumerate(gt_mesh_colored_o3d.triangles):
            # print('raw_label[triangle[0]]', raw_label[triangle[0]])
            if raw_label[triangle[0]] in [1, 2, 3]: # ceiling floor wall
                # 获取三角面的顶点
                v0 = np.asarray(gt_mesh_colored_o3d.vertices)[triangle[0]]
                v1 = np.asarray(gt_mesh_colored_o3d.vertices)[triangle[1]]
                v2 = np.asarray(gt_mesh_colored_o3d.vertices)[triangle[2]]
                # 计算平面的法向量
                normal = np.cross(v1 - v0, v2 - v0)
                normal = normal / np.linalg.norm(normal)
                # 计算点到平面的距离
                for i, pt in enumerate(gt_ds):
                    distance = np.abs(np.dot(normal, pt[:3] - v0))
                    if distance < distance_threshold and raw_label[triangle[0]] != pt[-1] and self.is_point_in_triangle(pt[:3], v0, v1, v2):
                        # print(f"Point {pt} is within the plane of triangle {i} with distance {distance},raw_label:{pt[-1]}->new_label{raw_label[triangle[0]]}")
                        if raw_label[triangle[0]] in [1, 3] and pt[-1] == 2:
                            continue
                        gt_ds[i, 3:] = np.array([raw_label[triangle[0]]] * 3)
    ...
    def _gt_compile(self, scene_model):
        ...
        for obj_id in range(len(scene_model.objects)):
            if scene_model.objects[obj_id].model.label == "floor":
                floor_id = obj_id
            scene_model.objects[obj_id].model.label = self._class_mapping(scene_model.objects[obj_id].model.label)
            # break
        ...
        # print("顶点颜色:"*20, np.asarray(mesh_o3d.vertex_colors))
        ...
        _label = vertices[:, -1]
        return pcd, pcd_crop, mesh_o3d, _label, cam_pose, vox_origin_inwld
    ...
    def _class_mapping(self, label) -> np.ndarray:
        obj_nyu894_id = int(ismember([label], self.nyu894_class)[1].item())
    ...
    def _save_data(self, save_dir, scene, **save_options):
        ...
        if visual_opts.get("mesh_colored") is True:
...
if __name__ == "__main__":
    nyucad_preprocess = NYUCADPreprocessing(n_jobs=32)
